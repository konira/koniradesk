...
#[test]
fn test() {
    let mut manager = DXGIManager::new(10).unwrap();   
    ...
    let mut child = std::process::Command::new("ffmpeg")     
                                                    ...
                                                    .arg("-f")
                                                    .arg("mpegts")
                                                    .arg("udp://localhost:1234")
                                                    .stdin(std::process::Stdio::piped())
                                                    .spawn().unwrap();
    let stdin = child.stdin.as_mut().unwrap();
    let start = std::time::Instant::now();
    while start.elapsed().as_secs() < 10000  {
        ...
        count+=1;
        print!("count frames {} | secs {}\n", count, start.elapsed().as_secs());
        std::io::stdout().flush().unwrap();
    }
    println!("Total Frames {} | Total secs: {}",count, start.elapsed().as_secs());   
    assert!(count >= 1);
